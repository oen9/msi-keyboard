...
class KeyboardUsbConnectorImpl extends KeyboardUsbConnector {
  private val LOGGER: Logger = Logger.getLogger(classOf[KeyboardUsbConnectorImpl])
  ...
  protected def createDevicehandle(vendorId: Short, productId: Short): DeviceHandle = {
    val device = findDevice(vendorId, productId).getOrElse(throw new DeviceNotFoundException(vendorId, productId))
  ...
  protected def findDevice(vendorId: Short, productId: Short): Option[Device] = {
    ...
    try {
      for (device <- list) {
        ...
        if (descriptor.idVendor == vendorId && descriptor.idProduct == productId) return Some(device)
      }
